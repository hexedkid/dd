...
class TLC59116_Unmanaged {
  ...
  public: // Low-Level constants
    ...
    static const byte MODE1_Register = 0x00;
      ...
      static const byte MODE1_SUB2_mask = 0b0100;
      static const byte MODE1_SUB3_mask = 0b0010;
    ...
    static const byte LEDOUT0_Register = 0x14; // len_num -> LEDOUTx_Register number. Registers are: {0x14, 0x15, 0x16, 0x17}
      ...
      static byte LEDx_Register_mask(byte led_num) { // bit mask for led mode bits; возвращает "11" в соответствующих леду битах (регистра LEDOUTx)
        LEDOUTx_CHECK(__LINE__, led_num); 
        return LEDOUT_Mask << (2 * (led_num % 4)); 
        }
      static byte LEDx_Register_bits(byte led_num, byte register_value) { // extract bits for led; соответствие маски "11" данного леда и значения register_value?
        LEDOUTx_CHECK(__LINE__, led_num);
	  ...
	  // возвращает байт с битами указанными в out_mode в позиции регистра соответствующей леду
	  
      static byte LEDx_set_mode(byte was, byte led_num, byte mode) {
        LEDOUTx_CHECK(__LINE__, led_num);
        return set_with_mask(was, LEDx_Register_mask(led_num), LEDx_to_Register_bits(led_num, mode)); // в чем менять/маска/биты (was/11 в позиции леда/mode)
        }
